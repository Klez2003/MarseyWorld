...
def stats():
	...
	stats = {
			...
			"Users with a private post history": "{:,}".format(g.db.query(User).filter_by(private_posts=True).count()),
			"Users with a private comment history": "{:,}".format(g.db.query(User).filter_by(private_comments=True).count()),
